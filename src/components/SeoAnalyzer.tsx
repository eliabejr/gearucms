			...
			<div className="mb-4 flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					<div className="h-2.5 w-24 overflow-hidden rounded-full bg-[var(--sand)]">
						<div
							className={`h-full rounded-full transition-all ${scoreBg(seo.score)}`}
							style={{ width: `${seo.score}%` }}
						/>
					</div>
					<span
						className={`text-lg font-bold ${scoreColor(seo.score)}`}
					>
						{seo.score}
					</span>
				</div>
				...
				{seo.checks.map((check) => (
					...
					>
						...
						<div>
							...
							<span
								className={`ml-1 ${statusColors[check.status]}`}
							>
								— {check.message}
							</span>
						</div>

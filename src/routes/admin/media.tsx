...
function MediaPage() {
	...
	const { data: mediaList, isLoading } = useQuery(
		trpc.media.list.queryOptions({ limit: 100 }),
	)
	...
	return (
		<div>
			...
			<div
				...
				role="button"
				tabIndex={0}
				className={`island-shell mb-6 flex cursor-pointer flex-col items-center justify-center gap-2 border-2 border-dashed p-8 transition ${
					isDragging
			...
			{isLoading ? (
				...
				<div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
					{mediaList.map((item) => (
						<div
							key={item.id}
							className="island-shell group relative overflow-hidden"
						>
							{/* Thumbnail */}
							...
							<div className="absolute inset-x-0 top-0 flex justify-end gap-1 p-2 opacity-0 transition group-hover:opacity-100">
								<button
									...
									className="rounded-lg bg-white/90 p-1.5 text-[var(--sea-ink-soft)] shadow-sm transition hover:text-[var(--lagoon)]"
									title="Copy URL"
								>
									{copiedId === item.id ? (
										<Check size={14} />

...
function LeadsTab() {
	...
	const { data: forms } = useQuery(trpc.leads.listForms.queryOptions())
		...
		trpc.leads.deleteLead.mutationOptions({
			onSuccess: () => {
				queryClient.invalidateQueries({
					queryKey: trpc.leads.listLeads.queryKey(),
				})
			},
		}),
	)

	// Get unique tags from forms
			...
			<div className="mb-4 flex flex-wrap items-center gap-3">
				<div className="min-w-[200px]">
					<Select
						...
						onChange={(val) =>
							setFilterFormId(val ? Number(val) : undefined)
						}
						options={(forms ?? []).map((f) => ({
							value: String(f.id),
							label: f.name,
...
function FormsTab() {
	...
	const { data: forms, isLoading } = useQuery(
		trpc.leads.listForms.queryOptions(),
	)
	...
	const invalidateForms = () => {
		queryClient.invalidateQueries({
			queryKey: trpc.leads.listForms.queryKey(),
		})
	}
						...
						<div>
							...
							{formFields.length === 0 ? (
								<p className="py-4 text-center text-sm text-[var(--sea-ink-soft)]">
									No extra fields. Only name and email will
									be collected.
								</p>
							) : (
			...
			{isLoading ? (
				...
				<div className="island-shell py-12 text-center">
					<div className="flex flex-col items-center gap-2">
						...
						/>
						<p className="text-[var(--sea-ink-soft)]">
							No lead forms yet. Create your first one!
						</p>
					</div>
				</div>
			) : (
								...
								<div className="flex gap-1">
									...
									<button
										...
										onClick={() => {
											if (
												confirm(
													`Delete form "${form.name}"? This will also delete all its leads.`,
												)
											) {
												deleteMutation.mutate({
													id: form.id,

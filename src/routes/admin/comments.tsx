...
function CommentsPage() {
	...
	const { data: commentsList, isLoading } = useQuery(
		trpc.comments.list.queryOptions({ status: activeTab, limit: 100 }),
	)

	const moderateMutation = useMutation(
		trpc.comments.moderate.mutationOptions({
			onSuccess: () => {
				queryClient.invalidateQueries({ queryKey: trpc.comments.list.queryKey() })
			},
		}),
	)
	...
	return (
		<div>
			...
			{/* Tabs */}
			<div className="mb-4 flex rounded-lg border border-[var(--line)] bg-[var(--sand)] w-fit">
				{statusTabs.map((tab) => (
					<button
						key={tab.value}
						type="button"
								...
								<div className="flex-1">
									...
									<div className="flex flex-wrap items-center gap-3 text-xs text-[var(--sea-ink-soft)]">
										<span>
											on{" "}
											<span className="font-medium text-[var(--sea-ink)]">
												{comment.entry?.title ?? "Unknown Entry"}
											</span>
										</span>
										<span>
											{comment.createdAt
								...
								<div className="flex gap-1">
									{activeTab !== "approved" && (
										<button
											...
											className="rounded-lg p-2 text-[var(--sea-ink-soft)] transition hover:bg-green-50 hover:text-green-600"
											title="Approve"
										>
											<Check size={16} />
										</button>
									...
									{activeTab !== "rejected" && (
										<button
											...
											disabled={moderateMutation.isPending}
											className="rounded-lg p-2 text-[var(--sea-ink-soft)] transition hover:bg-orange-50 hover:text-orange-600"
											title="Reject"
										>
											<X size={16} />
										</button>

...
export function EntryId() {
	...
	const trpc = useTRPC() as {
		entries: {
			...
			restoreVersion: { mutationOptions: (opts: { onSuccess: () => void }) => unknown }
		}
	}
	...
	useEffect(() => {
		...
		if (e) {
			...
			setOgImage(e.ogImage ?? "")
			const values: Record<number, string | null> = {}
			for (const ef of e.fields ?? []) values[ef.fieldId] = ef.value
			setFieldValues(values)
		}
	}, [entry])

	const invalidate = () => {
		queryClient.invalidateQueries({ queryKey: trpc.entries.getById.queryKey({ id }) })
		queryClient.invalidateQueries({ queryKey: trpc.entries.getVersions.queryKey({ entryId: id }) })
	}
	const updateMutation = useMutation(trpc.entries.update.mutationOptions({ onSuccess: invalidate }))
	const updateStatusMutation = useMutation(trpc.entries.updateStatus.mutationOptions({ onSuccess: invalidate }))
	const restoreVersionMutation = useMutation(trpc.entries.restoreVersion.mutationOptions({ onSuccess: invalidate }))
	...
	const handleRestore = (versionId: number) => {
		if (confirm("Restore this version? Current unsaved changes will be lost.")) restoreVersionMutation.mutate({ entryId: id, versionId } as any)
	}
	...
	const renderField = (field: { id: number; name: string; type: string; required: boolean | null }) => {
		...
		switch (field.type) {
			...
			case "boolean":
				return (
					<label className="flex items-center gap-2 text-sm text-[var(--sea-ink)]">
						<input type="checkbox" checked={value === "true"} onChange={(e) => setFieldValue(field.id, e.target.checked ? "true" : "false")} className="h-4 w-4 rounded border-[var(--line)] accent-[var(--lagoon)]" />
						{field.name}
					</label>
				)
			case "image":
			...
			default:
				return <input type="text" value={value} onChange={(e) => setFieldValue(field.id, e.target.value)} className={inputClass} />
		}
	}
	...
	return (
		<div>
			<button type="button" onClick={() => navigate(`${basePath}/entries`)} className="mb-4 flex items-center gap-1 text-sm text-[var(--sea-ink-soft)] transition hover:text-[var(--sea-ink)]">
				<ArrowLeft size={16} />
				Back to Entries
			</button>
			...
			<div className="island-shell mb-6 p-5">
				<div className="flex flex-col gap-4">
					<div>
						<label htmlFor="entry-title" className="mb-1 block text-xs font-medium text-[var(--sea-ink-soft)]">Title</label>
						<input id="entry-title" type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-lg border border-[var(--line)] bg-[var(--foam)] px-3 py-2 text-lg font-semibold text-[var(--sea-ink)] outline-none focus:border-[var(--lagoon)]" />
					</div>
					<div>
						<label htmlFor="entry-status" className="mb-1 block text-xs font-medium text-[var(--sea-ink-soft)]">Status</label>
						<div className="max-w-xs">
							<Select id="entry-status" value={status} onChange={(val) => handleStatusChange(val as "draft" | "published" | "archived")} options={[{ value: "draft", label: "Draft" }, { value: "published", label: "Published" }, { value: "archived", label: "Archived" }]} isSearchable={false} />
						</div>
			...
			<div className="island-shell mb-6 p-5">
				<h2 className="mb-4 text-lg font-semibold text-[var(--sea-ink)]">Fields</h2>
				{collectionFields.length === 0 ? (
					<p className="py-4 text-center text-sm text-[var(--sea-ink-soft)]">No fields defined for this collection.</p>
				) : (
					<div className="flex flex-col gap-4">
						{collectionFields.map((field) => (
							<div key={field.id}>
								{field.type !== "boolean" && (
									<label htmlFor={`field-${field.id}`} className="mb-1 flex items-center gap-1 text-xs font-medium text-[var(--sea-ink-soft)]">
										{field.name}
										{field.required && <span className="text-red-500">*</span>}
										<span className="ml-1 rounded-full bg-[var(--foam)] px-1.5 py-0.5 text-[10px] text-[var(--sea-ink-soft)]">{field.type}</span>
									</label>
								)}
								{renderField(field)}
							</div>
			...
			<div className="island-shell p-5">
				<button type="button" onClick={() => setShowSeo(!showSeo)} className="flex w-full items-center justify-between text-left">
					...
					<ChevronDown size={18} className={`text-[var(--sea-ink-soft)] transition ${showSeo ? "rotate-180" : ""}`} />
				</button>
				{showSeo && (
					<div className="mt-4 flex flex-col gap-4">
						<div>
							<label htmlFor="meta-title" className="mb-1 flex items-center justify-between text-xs font-medium text-[var(--sea-ink-soft)]">
								Meta Title
								<span className={metaTitle.length > 60 ? "text-red-500" : ""}>{metaTitle.length}/60</span>
							</label>
							<input id="meta-title" type="text" value={metaTitle} onChange={(e) => setMetaTitle(e.target.value)} placeholder={title || "Defaults to entry title"} className="w-full rounded-lg border border-[var(--line)] bg-[var(--foam)] px-3 py-2 text-sm text-[var(--sea-ink)] outline-none focus:border-[var(--lagoon)]" />
						</div>
						<div>
							<label htmlFor="meta-description" className="mb-1 flex items-center justify-between text-xs font-medium text-[var(--sea-ink-soft)]">
								Meta Description
								<span className={metaDescription.length > 160 ? "text-red-500" : ""}>{metaDescription.length}/160</span>
							</label>
							<textarea id="meta-description" value={metaDescription} onChange={(e) => setMetaDescription(e.target.value)} placeholder="Auto-generated from content if empty" rows={3} className="resize-y rounded-lg border border-[var(--line)] bg-[var(--foam)] px-3 py-2 text-sm text-[var(--sea-ink)] outline-none focus:border-[var(--lagoon)]" />
						</div>
						<div>
							<label htmlFor="og-image" className="mb-1 block text-xs font-medium text-[var(--sea-ink-soft)]">OG Image URL</label>
							<input id="og-image" type="text" value={ogImage} onChange={(e) => setOgImage(e.target.value)} placeholder="Auto-generated if empty" className="w-full rounded-lg border border-[var(--line)] bg-[var(--foam)] px-3 py-2 text-sm text-[var(--sea-ink)] outline-none focus:border-[var(--lagoon)]" />
						</div>
						<div className="rounded-lg border border-[var(--line)] bg-white p-4">
							...
							<p className="text-sm text-[#006621]">example.com/{e.collection?.slug ?? "..."}/{e.slug}</p>
			...
			/>
			<div className="island-shell p-5">
				<button type="button" onClick={() => setShowVersions(!showVersions)} className="flex w-full items-center justify-between text-left">
					<div className="flex items-center gap-2">
						<History size={18} className="text-[var(--sea-ink-soft)]" />
						<h2 className="text-lg font-semibold text-[var(--sea-ink)]">Version History</h2>
					</div>
					<span className="text-sm text-[var(--sea-ink-soft)]">{showVersions ? "Hide" : "Show"}</span>
				</button>
				{showVersions && (
					<div className="mt-4">
						{!versions?.length ? (
							<p className="py-4 text-center text-sm text-[var(--sea-ink-soft)]">No versions yet.</p>
						) : (
							<ul className="flex flex-col gap-2">
								{(versions as { id: number; versionNumber: number; createdAt?: string }[]).map((version) => (
									<li key={version.id} className="flex items-center justify-between rounded-lg border border-[var(--line)] px-4 py-3">
										<div>

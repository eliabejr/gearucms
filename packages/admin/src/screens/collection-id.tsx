				...
				{!col.fields?.length ? (
					...
					<ul className="flex flex-col gap-1">
						{col.fields.map((field) => (
							<li key={field.id} className="flex items-center gap-3 rounded-lg px-3 py-2.5 transition hover:bg-[var(--foam)]">
								<GripVertical size={16} className="text-[var(--sea-ink-soft)] opacity-40" />
								<div className="flex-1">
								...
								<span className="rounded-full bg-[var(--foam)] px-2 py-0.5 text-xs font-medium text-[var(--sea-ink-soft)]">{field.type}</span>
								{field.required && <span className="rounded-full bg-red-50 px-2 py-0.5 text-xs font-medium text-red-600">required</span>}
								<button
									type="button"
									onClick={() => {
